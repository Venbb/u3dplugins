...
public class WXAPIController
{
	...
	{
		Constants.SHARE_TITLE = title;
		Constants.SHARE_DESC = desc;
		Constants.SHARE_ICON_PATH = iconPath;
		Constants.SHARE_URL = url;
		Handler handler = new Handler(Looper.getMainLooper());
		handler.post(new Runnable()
		{
			@Override
			public void run()
			{
				File file = new File(Constants.SHARE_ICON_PATH);
				if (!file.exists())
				{
					Log.d(TAG, "未能找到图标：" + Constants.SHARE_ICON_PATH);
				}

				WXImageObject imgObj = new WXImageObject();
				imgObj.setImagePath(Constants.SHARE_ICON_PATH);
				...
				msg.description = Constants.SHARE_DESC;
				msg.title = Constants.SHARE_TITLE;

				Bitmap bmp = BitmapFactory.decodeFile(Constants.SHARE_ICON_PATH);
	...
	// 发送消息给Unity
	public static void SendMessage(String dataStr, String ckFun)
	{
